...
## The player character is being rebuilt

On **2026-08-24** the ranger's mesh, rig, clips and asset pipeline were deleted, to be rebuilt
from new source files. What used to be written here - the measured stride lengths, the covers
that drive playback rate, which gait was owed a rebuild - was true of that character and is not
true of anything now.

What the replacement still has to satisfy is unchanged, and is worth stating before it arrives:

* **A clip's `covers` drives its playback rate.** `lasts * speed / covers` is the single most
  consequential constant in movement, and a value belonging to a different animation is exactly
  what "running through water" looks like. It is measured off the file, never typed.
* **Movement is judged against Genshin Impact, not against realism.** A realism band must never
  gate a tuning value; a value a guard can refuse is an output, not a knob.
* **The limbs must bend the right way, and something has to refuse it if they do not.** Three
  gaits shipped with backwards knees and every one was caught by the person playing the game.
