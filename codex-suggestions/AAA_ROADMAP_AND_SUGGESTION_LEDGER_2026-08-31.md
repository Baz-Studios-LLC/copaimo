...
| AQ-027 | P1 | Road handoff | A country road now ENDS at the town boundary and nothing continues it: `790cae4` clips at `town_reaches` on the promise that the town draws the continuation, but `lay_out` only uses those roads to keep buildings off the line. Measured at the city at (-2553, 1771): dirt ends 320 m out, nearest street point 126.2 m further on. Found by photographing a handoff while looking for AQ-022. Three fix shapes tried and reverted - per-segment ways make a chain of junction discs; mismatched `wide`/`joins` tears the ribbon; and the town's high street is built along `approach`, so a naive continuation duplicates it. | open, mine, not started | The high street already IS the continuation and stops short of the clip. Run it out to `town_reaches` on the approach bearing (preferred - one owner per piece of ground), or clip the country road where the town's network actually starts. Prove with a per-road gap measurement, not a furthest-street one. |
