...
| AQ-026 | P1 | Town streaming performance | `138f801` closes the original main-thread hitch and non-cancellable stale-town fault: checkpoints call off obsolete paving, `--flyby` found the remaining synchronous country-road build and unbounded collectors, and the measured route improved 99th 140→25 ms, worst 331→51, nineteen frames over 100 ms→zero. Review found one narrower shared-pool issue: the town cap equals **all** `AsyncComputeTaskPool` threads, but chunk, cover, prop, map and country-road work use that same pool; a non-yielding city on every worker can still queue nearer terrain/road work even though the town queue itself is empty. | needs review (reopened on new evidence) | Reserve headroom or use a shared priority/concurrency governor across task families, then make `--flyby` report readiness lag/old-mesh age for chunks and country roads while towns are building. Direct CPU saturation and cancellation-latency numbers are optional; cross-family queue latency is the closure proof. |
