...
| AQ-024 | P1 | Authored kerb ink | Both review findings closed. Non-road UV1 contamination was already guarded - specialization returns early unless the mesh carries `ATTRIBUTE_KERB_STANDS`, with two tests. Prepasses checked empirically: depth+normal+motion enabled together on the real camera, city rendered at eye level, custom interstage compiles and draws, no validation errors. The 0.35->0.75 gateway is now a test against a REAL approach, reading the vertex attribute the shader reads; red/green proven (38 offending vertices with the old `stone_contrast` gate, zero with the kerb's own arrival). Its first version proved nothing and its own guard caught that. | closed pending your review | Remaining from your list: moving multi-resolution approach capture, and the primary-kerb vs outer-footway hierarchy - the latter is an art call, going to the user with pictures rather than tuned blind. |
...
| AQ-026 | P1 | Town streaming performance | Reopened finding confirmed and fixed in `36d691a`. `--flyby` now times the GROUND's wait, and on the shipped code that was median 42 ms / 95th 615 / worst 1537 with the frame rate perfect throughout - frame time alone could not have found it. Towns now take a quarter of the pool, never fewer than one; ground 95th falls to ~350-500 ms, median to 24-31. Recorded caveat: caps of 4, 3 and 2 never bound on that route (max concurrency was 2), so those three readings were one experiment and their differences were noise. | closed pending your review | Cross-family latency is now reported. Reopen if you want country-road readiness lag as its own number, or a real governor rather than a fixed share. |
