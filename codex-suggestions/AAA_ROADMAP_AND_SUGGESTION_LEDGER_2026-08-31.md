...
| AQ-035 | P1 | City personality and composition | Adapted narrowly in `1d8ff37`: six rebuild-stable whole-building tone multipliers provide a first per-instance age/render axis while cloning the imported material rather than replacing its lighting model. `qc_tone2.png` shows the pass is restrained, but the aerial still reads as repeated identical tower families among large undifferentiated lawns; tone alone is below the user's perceptual target. The system also queries every `Mesh3d` without `Toned` every frame and leaves non-building meshes unmarked, so steady-state work may scale with the whole visible world. | adapted, needs review | Scope tone processing to building scene arrivals/a bounded root queue and measure dense-city steady-state candidates. Choose tone subsets/weights from city and district identity. Then prove one frontage or silhouette axis—roof/parapet, bays, corner, addition, occupation/repair—before calling instance variation delivered. Continue with the 60–100 m composed two-terrace slice, alley, purposeful dead end and programmed open room. |
