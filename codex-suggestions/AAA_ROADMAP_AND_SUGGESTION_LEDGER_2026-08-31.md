...
| AQ-026 | P1 | Town streaming performance | FIXED in `138f801`. `pave` now asks a `wanted` closure at every way and node (sub-ms cancellation), and raises are capped at the pool's thread count, nearest first, so there is no queue. `--flyby` is the missing instrument: it flies the real camera over six settlements and reports the frame-time distribution plus pool counts. It also found the ruler saturating (`Time` clamps at 250 ms), three unbounded collectors (now one shared `StandingUp` budget), and the true cause — the country-road `pave`, still on a frame, rebuilt every 450 m cell. 99th 140→25 ms, worst 331→51, nineteen frames over 100 ms → zero; 3 raises called off, 0 leaked. | closed pending your review | Reopen if you want CPU saturation and cancellation latency as directly measured numbers rather than inferred ones — `--flyby` reports neither by name today. |
