...
|---|---|---|---|---|---|
| AQ-001 | P0 | Test evidence | Make `--drive` arrivals and blockers semantic. Current arrival is a 1.2 m radius; any 0.75 s no-progress or timeout passes `Blocked`, even at the wrong obstacle. Resurfaced once after `0516940` and `faa26a9` repeated “33/33” without changing the oracle. | needs review | Record a disposition; add finish regions/intended-barrier contact bands when scheduled, and meanwhile qualify the 33/33 claim as using the current radius/stall oracle. |
...
| AQ-020 | P1 | Clouds | `0516940` imports Bevy’s unlit flag but never reads it; the custom fragment still calls PBR lighting unconditionally, so the committed cloud fix is a no-op. | needs review | Mirror Bevy 0.16’s unlit branch (or explicit cloud-marker branch) and prove the same dusk view has no specular highlight while terrain remains rendered. |
| AQ-021 | P0 | Ink regression | `faa26a9` commits `much * 0.0` in the final ink blend, globally disabling screen-space outlines. It remains present after `2f1a948`, `69310d0` and `3b9ec57`; resurfaced once at that threshold. | needs review | Restore the nonzero blend and add a standing nonzero-coverage check before accepting further visual evidence; until then, do not cite the outline pass as contributing to road/kerb images. |
...
| AQ-023 | P1 | Kerb continuity | `3b9ec57` correctly removes the ribbon's metre-wide painted edge gradient and gives its real kerb face a darker material. Junction nodes still paint stations `1..=3` with `ROAD_KERB`, however: their carriageway-to-foot band remains a broad road→kerb gradient and their actual face never receives `ROAD_KERB_FACE`. | needs review | Mirror the ribbon's station semantics at nodes (road/grain at the face foot, dark face at its top, kerb top behind it) and add a ribbon-to-node colour/grain continuity guard plus an oblique junction still. |
