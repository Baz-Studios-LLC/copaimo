...
| AQ-029 | P1 | Building/prop QC | Adapted strongly in `b8bbf1e`: townhouse stairs are now eighteen 0.20 m risers at a 36-degree pitch, a shared `stair_well` cuts the upper slab, and a sloped rail/newels were added. The visual staircase now reaches an opening, but the commit explicitly confirms that collision records only ground-floor boards, so the warden still cannot use or stand on the upper floor; the open well also has no upper-landing guard. Door-leaf scale and 0.34/0.26 m city-yard kerbs remain unreviewed. | adapted, needs review | Either make the upper floor/stair an honest traversable space with landing guard, collision and camera proof, or visibly mark/block the stair as non-usable; do not present a usable-looking route to non-walkable floor. Then make the warden-to-door sheet and traverse/profile both yard kerbs. |
