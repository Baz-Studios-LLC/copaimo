...
| AQ-024 | P1 | Authored kerb ink | Claude accepted both review findings. The active follow-up adds a road-only `ATTRIBUTE_KERB_STANDS`, specializes the shader only when that attribute exists, and gates line strength on the actual kerb curve rather than `stone_contrast`. This directly addresses phantom gateway lines and non-road UV1 contamination. | accepted | Before closure, compile/render the custom interstage in the normal main pass and enabled depth/normal/motion prepasses, then capture moving multi-resolution approach/junction evidence and tune primary kerb versus outer-footway line hierarchy. |
| AQ-025 | P1 | Player traversal | User-requested pre-mount hoverboard. A Copaimo-specific Blender/gameplay spec and two Warden-matched visual references define the foldable backpack prop, deploy/ride/stow clips, event handoffs, kinematic surface/collision sharing, deck IK and first-slice proof. | accepted by user; needs Claude scheduling | Claude disposition and implementation boundary; close the first slice only with an uncut on-foot→deploy→ride→brake→stow capture, no duplicate/pop invariant, multi-point board collision, invalid-surface recovery and stable 30/60/120 Hz timing. |
| AQ-026 | P1 | Town streaming performance | `f294481` correctly removes 0.3–6 s settlement construction from the main frame, but its spawned `async move` performs the entire synchronous layout/pave job without an await or cancellation checkpoint. Dropping its `Task` prevents another poll; it cannot preempt the one poll already executing the whole job, so rapid editor flight can leave obsolete city work consuming the pool. | needs review | Add cooperative cancellation/generation checks at bounded layout/mesh stages plus an explicit concurrency/priority limit, then measure a rapid multi-town fly route: frame-time distribution, active/queued/stale jobs, cancellation latency and CPU saturation—not only stationary FPS. |
