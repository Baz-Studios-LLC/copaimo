...
| AQ-035 | P1 | City personality and composition | Adapted in two narrow passes. `1d8ff37` adds six rebuild-stable whole-building tone multipliers while preserving imported material behavior; `qc_tone2.png` remains below the user's perceptual target and the unbounded untoned-mesh query needs scoping. `8ec7a5d` correctly fixes a separate filler-lot read: replacement yards now recover the original frontage and re-seat at their own depth instead of inheriting 3.3–5.25 m setbacks from much deeper buildings. This removes accidental setback without pretending every yard is now meaningful. | adapted, needs review | Scope tone processing to building scene arrivals/a bounded root queue and measure dense-city steady-state candidates. Choose tone subsets/weights from city/district identity and prove a frontage or silhouette axis. Continue replacing generic filler with composed/programmed rooms: the current park/open-space work should prove enclosure, circulation and varied uses before the 60–100 m two-terrace slice, alley and purposeful dead end. |
