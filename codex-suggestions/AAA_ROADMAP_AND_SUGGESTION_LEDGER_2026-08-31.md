...
| AQ-031 | P1 | Spatial index | Codex's cause-level diagnosis was exact: `settle::CELL` sized the index AND was `approach()`'s search radius, so a 64 m cell turned every town. Split into `CELL = 64` and `APPROACH_WITHIN = 512`. Both guards that failed pass; desert guard passes; worst city paving 1,719 -> 297 ms at 4x the buildings; audit clean, bot 33/33, 365 tests on one commit. | fixed - pending your equivalence check | Your step 3 (two-resolution semantic-equivalence regression) is not written yet; the world-unchanged evidence today is the guards, not a direct comparison. Say if you want that before closing. |
| AQ-032 | P1 | Pedestrian city coherence | Adapted in current dirty work: population cap 420→300 and the explicit car ramp is removed. `qc_deck.png` still reads as an empty parking frame: five unoccupied decks, first-deck-only visible stair, no companion/service programme, and detached-looking banner panels. The aerial still has broad accidental lawn rather than named open-space parcels. | adapted, needs review | Prove a ground + first-terrace exchange with honest arcade, accessible circulation/guards, companion water/rest, porter/service route and occupied edges; reserve programmed commons/open ground before lot filling rather than tuning population alone. Detailed specs: `PEDESTRIAN_CITY_BUILDINGS_PROPS_COLOR_SPEC_2026-09-02.md` and AQ-033 brief. |
