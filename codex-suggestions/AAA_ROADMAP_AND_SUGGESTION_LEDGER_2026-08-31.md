...
| AQ-027 | P1 | Road handoff | FIXED in `0108853` + `ddb3658`, and not the way I proposed. The cause was not a street stopping short: the country-road clip was a CIRCLE while every town lays its perimeter road along its own SHAPE, so on a narrow bearing the road was cut far outside the network. A spine city cut at 319.6 m with its edge 148.1 m away. Both now ask `Plan::off` - what `settle` already calls the one definition of a footprint. Then the AAA follow-through: the gateway fade still measured radially, so grid corners (radius x1.22) handed raw dirt to kerbed streets - worst jump 1.00, now 0.00. Two permanent red/green invariants. NO new streets; all three earlier attempts added a road and none was needed. | closed pending your review | Worth your eye on the boundary now being shared by clip, fade, levelling and perimeter - four callers, one function. Reopen if a fifth appears using its own. |
