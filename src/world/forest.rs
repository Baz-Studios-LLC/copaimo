//! Where the trees are, as the game reads it.
//!
...
impl Painted {
    ...
    pub fn load(half: Vec2) -> Self {
        Self::load_from(Path::new(FOREST_PATH), half)
    }
        ...
        if wide != empty.wide || deep != empty.deep || saved_half.distance(half) > 1.0 {
            warn!(
                "{} was painted for a {:.0}x{:.0} m world, not this {:.0}x{:.0} m one \
                 - ignoring it. Re-plant it at Opificium's terrain bench.",
                path.display(),
                saved_half.x * 2.0,
                saved_half.y * 2.0,
                half.x * 2.0,
...
mod tests {
    ...
    fn painting_forces_the_question_either_way() {
        assert!(density(0.0, 1.0) > 0.99);
        assert!(density(1.0, -1.0) < 0.01);
        for natural in [0.0, 0.25, 0.5, 0.75, 1.0] {
            assert_eq!(density(natural, 0.0), natural);
        }
    }

    /// The numbers below were taken from Opificium's own `chance`, and this is
    /// the test that catches the two drifting apart.
    ///
    /// A scatter that differs by one digit gives the bench one forest and the
    /// game another with nothing failing anywhere — so it is pinned to literals
    /// rather than to a second copy of the same arithmetic, which would agree
    /// with itself no matter how wrong it was.
    #[test]
    fn the_scatter_matches_the_bench_exactly() {
        // Printed by Opificium's `print_the_scatter`, which exists to produce
        // exactly these. If the hash ever changes on purpose, run it again and
        // paste the new numbers here.
        for (x, z, salt, expected) in [
            (0, 0, 1u32, 0.427_846_25_f32),
            (0, 0, 3, 0.677_951_81),
            (17, -400, 3, 0.818_481_45),
            (-219, 47, 4, 0.554_404_44),
        ] {
            let got = chance(x, z, salt);
            assert!(
                (got - expected).abs() < 1.0e-6,
                "chance({x}, {z}, {salt}) is {got:.8} here and {expected:.8} at the bench \
                 - the two forests have parted company"
            );
        }
    }
